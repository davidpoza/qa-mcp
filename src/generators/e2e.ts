...
function referenceSkeleton(): string {
  return [
    ...
    "    // Interacción real derivada del código frontend. Para desplegables custom",
    "    // (p. ej. <empresas-ui-dropdown id='sociedad'>) pasa el contenedor al helper:",
    "    selectFirstSelectableOption(\"#sociedad\"); // resuelve el <select> nativo interno",
...
function buildE2EGenerationPrompt(params: {
  ...
  return [
    ...
    "   - Para LEER opciones (contar, validar vacío) usa `getSelectOptions('#sociedad').then((opts) => { ... })`. NUNCA uses `cy.get('#sociedad').find('option')` porque `.find` reintenta hasta que exista una opción y COLGARÁ el test en escenarios vacíos.",
    ...
    "- CU de error/vacío (500, 404, `[]`): haz **stub** con `cy.intercept(method, glob, { statusCode, body })`, dispara la acción y verifica un efecto observable QUE EXISTA en el código: cambio de ruta (p. ej. `cy.url().should('include', '/error')` si el routing redirige a una pantalla de error), un desplegable sin opciones seleccionables (compruébalo con `getSelectOptions(...).then(...)`, nunca con `.find('option')`), controles deshabilitados, o un mensaje cuyo selector/clase aparezca realmente en las plantillas. NO asumas clases genéricas como `.error-message` o `.alert-danger` si no están en el código.",
